...
var create = function create() {
  ...
  var addActivity = function addActivity(q) {
    var accessToken = (0, _booshReactAuth.getAccessToken)();
    if (accessToken) api.setHeader('Authorization', 'Bearer ' + accessToken);
    return api.post('activity', q);
  };

  var updateActivity = function updateActivity(id, q) {
    var accessToken = (0, _booshReactAuth.getAccessToken)();
    if (accessToken) api.setHeader('Authorization', 'Bearer ' + accessToken);
    return api.put('activity/' + id, q);
  };

  var getActivity = function getActivity(q) {
